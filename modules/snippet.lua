    ...
    if index and index > 0 then
        ...
        menubar:setMenu({
            ...
            { title = '-' },
            {
                title = '修改片段内容',
                fn = function()
                    result, text = hs.dialog.textPrompt(
                        '修改片段内容',
                        '请输入新的内容',
                        history[index].text,
                        '确定',
            ...
            },
            {
                title = '修改片段说明',
                fn = function()
                    result, subText = hs.dialog.textPrompt(
                        '修改片段说明',
                        '请输入新的说明',
                        history[index].subText,
                        '确定',
                        '取消'
                    )
                    if result == '确定' then
...
hs.hotkey.bind(hyperCmd, 'V', function()
    mChooser =
        chooser.new(completionFn):choices(history):rightClickCallback(rightClickCallbackFn):searchSubText(true):show()
end)
